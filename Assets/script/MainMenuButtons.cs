...
public class MainMenuButtons : MonoBehaviour
{
    ...
    {
        int x = PlayerPrefs.GetInt("Sound"); x = x == 1 ? 0 : 1; PlayerPrefs.SetInt("Sound", x); soundMnaager.instance.PlaySound(SoundName.CLICK);
        audioListener.enabled = x == 0;
        Color newColor = x == 1 ? Color.red : Color.white;
        ColorBlock cb = soundToggleButton.colors;
        cb.normalColor = newColor;
    ...
    {
        int pg = BookletAnimator.GetInteger("Page") + 1; soundMnaager.instance.PlaySound(SoundName.CLICK); if (pg == 4) pg = 3; BookletAnimator.SetInteger("Page", pg);
    }
